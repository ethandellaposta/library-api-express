...
export const return_book = (req: Request, res: Response, next: NextFunction) => {
  ...
  const updated_checkout = req.context.services.book_checkouts.update(checked_out_book.id, { returned_at: new Date() });
  req.context.services.books.update(book.id, { status: "available" });

  res.status(200).json(updated_checkout);
}
...
export const get_checked_out_books = (req: Request, res: Response) => {
  ...
  const user_checked_out_books = req.context.services.book_checkouts.find({
    user_id,
    returned_at: null
  });
